...
function 发布命令行脚本之范本集_主体功能 ({
    ...
    function 查找范文中的某标记并将真实的依赖包版本配置插入其后 ({
        ...
        if (产出的内容确系用于PowerShell代码) {
            ...
            let 命令诸行 = [
                '',
                `${唯一匹配到的标记文字所在行起始的空白}echo  '暂无。'`,
            ]



            if (确实须写入命令行脚本文件的内容的简易配置列表.length > 0) {
                ...
                命令诸行 = [
                    ...
                    `${唯一匹配到的标记文字所在行起始的空白}npm  i${npm命令行参数集 || ''} \\`,
            ...
            } // 这是 if (确实须写入命令行脚本文件的内容的简易配置列表.length > 0) 的终点。
